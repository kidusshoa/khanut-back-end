...
export const getDashboardStats = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (req.user.role === "business" && business.ownerId.toString() !== req.user.id) {
    ...
    const appointmentsChange = previousMonthAppointmentsCount === 0 
      ? 100 
      : ((totalAppointments - previousMonthAppointmentsCount) / previousMonthAppointmentsCount) * 100;
    ...
    const currentMonthCustomerIds = new Set([
      ...
      ...currentMonthAppointments.map(appointment => appointment.customerId.toString())
    ]);
    ...
    const previousMonthCustomerIds = new Set([
      ...
      ...previousMonthAppointments.map(appointment => appointment.customerId.toString())
    ]);
    
    const previousMonthCustomersCount = previousMonthCustomerIds.size;
    
    // Calculate customers change percentage
    const customersChange = previousMonthCustomersCount === 0 
      ? 100 
      : ((totalCustomers - previousMonthCustomersCount) / previousMonthCustomersCount) * 100;
  ...
  try {
    ...
    return res.status(200).json({
      ...
      datasets: [
        ...
        {
          label: period === 'week' ? 'Last Week' : period === 'month' ? 'Last Month' : 'Last Year',
...
export const getServiceDistribution = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const appointmentServices = services.filter(service => service.type === 'appointment').length;
    const productServices = services.filter(service => service.type === 'product').length;
    const uniformServices = services.filter(service => service.type === 'uniform').length;
    ...
    return res.status(500).json({ message: "Failed to get service distribution data" });
  }
};
...
export const getRecentOrders = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const formattedOrders = recentOrders.map(order => ({
      id: order._id,
      customer: order.customerId ? (order.customerId as any).name : 'Unknown Customer',
...
export const getUpcomingAppointments = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const formattedAppointments = upcomingAppointments.map(appointment => {
      ...
      return {
        id: appointment._id,
        customer: appointment.customerId ? (appointment.customerId as any).name : 'Unknown Customer',
        service: appointment.serviceId ? (appointment.serviceId as any).name : 'Unknown Service',
