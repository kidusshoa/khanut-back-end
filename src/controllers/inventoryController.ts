...
export const getBusinessInventory = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (lowStock === 'true') {
      query.stock = { $lte: 10 }; // Consider products with 10 or fewer items as low stock
    }
    ...
    return res.status(500).json({ message: "Failed to get business inventory" });
  }
};
...
export const updateProductStock = async (req: AuthRequest, res: Response) => {
  try {
    ...
    await ActivityLog.create({
      ...
      details: `Stock for "${product.name}" updated from ${previousStock} to ${product.stock}. Reason: ${reason || 'Not specified'}`,
    });
    ...
    try {
      ...
      for (const item of products) {
        ...
        if (product.businessId.toString() !== businessId) {
          throw new Error(`Product with ID ${productId} does not belong to this business`);
        }
...
export const getStockHistory = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Check if user owns this business
    if (req.user.role === "business" && business.ownerId.toString() !== req.user.id) {
      return res.status(403).json({ message: "Not authorized to view this product's history" });
    }
