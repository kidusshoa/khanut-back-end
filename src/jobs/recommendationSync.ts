...
export const runManualRecommendationSync = async (): Promise<boolean> => {
  try {
    logger.info(
      "Starting manual recommendation data sync and model retraining..."
    );
    ...
    if (success) {
      logger.info(
        "Manual recommendation data sync and model retraining completed successfully"
      );
    } else {
      logger.error(
        "Manual recommendation data sync and model retraining failed"
      );
    }
